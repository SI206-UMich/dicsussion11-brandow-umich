...
def create_patients_table(cur, conn):
    # Create new table for patients with the following fields:
    # Pet id, name(string), species_id(int), age(int), cuteness(int), aggressiveness(int)
    cur.execute('DROP TABLE IF EXISTS Patients')
    cur.execute('CREATE TABLE Patients (id INTEGER PRIMARY KEY, name TEXT, species_id INTEGER, age INTEGER, '
                'cuteness INTEGER, aggressiveness INTEGER)')
    conn.commit()
